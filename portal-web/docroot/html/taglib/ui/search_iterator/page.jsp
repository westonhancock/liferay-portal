	...
	<div id="<%= namespace + id %>SearchContainer">
		...
		%>
			...
			for (int j = 0; j < entries.size(); j++) {
				...
				if (headerNames != null) {
					...
					if (Validator.isNotNull(headerName) && !headerName.contains("hide-accessible")) {
						headerName = LanguageUtil.get(request, HtmlUtil.escape(headerName)) + StringPool.COLON;
					}
					else {
			...
			%>
				...
				<td aria-labelledby="<%= namespace + id %>_col-<%= normalizedHeaderName %>" class="table-cell <%= columnClassName %> text-<%= entry.getAlign() %> text-<%= entry.getValign() %>" colspan="<%= entry.getColspan() %>" tabindex="0">
